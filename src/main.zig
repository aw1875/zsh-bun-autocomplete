...
fn println(str: string) void {
    stdout.writer().writeAll(str) catch return;
}

fn add(str: string, output: *string) !void {
...
pub fn main() !void {
    ...
    if (args.len == 0) {
        println("run test repl init create install add remove update link unlink pm build upgrade");
        return;
    }

    var results: string = undefined;
    const query = if (args.len > 1) process_args[2] else "";

    if (commands.get(args[0])) |command| {
